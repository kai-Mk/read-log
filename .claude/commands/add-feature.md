...
1. mainブランチに切り替えて最新化: `git checkout main && git pull`
2. 日本語の機能説明から英語の機能名を推論する
3. ブランチ名を `{issue番号}-{機能名}` の形式で決定する
4. issueと紐づけてブランチを作成: `gh issue develop <issue番号> --name {ブランチ名} --checkout`
...
- [ ] issueの内容を確認した
- [ ] ブランチ `{issue番号}-{機能名}` をissueと紐づけて作成し、切り替えた
